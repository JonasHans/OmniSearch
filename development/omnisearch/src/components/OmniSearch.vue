<template>
	<b-container fluid>
		...
		<b-row>
			...
			<b-col lg="3">
				<column-chart v-if="dateHistory.length !== 0" :data="chartData"></column-chart>
				<wordcloud
					:data="wordCloud"
					nameKey="key"
					valueKey="score">
			...
			<b-col lg="6">
				<SearchResults :results="resultList"></SearchResults>
			</b-col>

			<b-col lg="3" v-if="resultList.length !== 0">
...
<script>
	...
	export default {
		...
		computed : {
			chartData: function() {
				let data = {}
				for (let i = 0; i < this.dateHistory.length; i++) {
					let hitDate = this.dateHistory[i]['key_as_string'].split(" ")[0]
					let hitCount = this.dateHistory[i]['doc_count']
